...
>
> 浙大短学期选课的同学进群备注：年级-专业-姓名（如：18-计算机-张三）；其他同学请备注：未选课-姓名；
...
1. 实现Reducing Snapshots to Points: A Visual Analytics Approach to Dynamic Network Exploration的可视化界面，实现下图中A B D部分，并完成如下功能（共80分）：
   ...
   > 助教会提供一个Demo，在本目录下的`vis`中，你可以进入该目录后，使用`npm i`安装依赖包，然后使用`npm run start`就可以启动该Demo。你将会看到左边有一个投影好的结果，右边则是随便选出来的某一个点所对应的图结构。暂时没有任何交互，代码逻辑也很混乱，仅仅提供给你们一个参考，希望各位可以自己去根据作业指南完成作业。在此过程中，助教将不遗余力为你们提供帮助，有任何问题都可以联系助教。
